...
/*大数相除判断*/
string Operation::Except_Judge(string a, string b,
    int n) //用来判断是大除小还是小除大
{
    if (a[0] == 0)
        return "0";
...
string Operation::Except_A(string a, string b, int n)
{
    ...
    if (n) //如果有小数位要求，将余数和除数作为被除数与除数进行小除大的运算
    {
        e = Except_B(a, b, n);
        e.erase(
            e.begin()); //最后返回的字符串形式为“0.xxxxxxxx”，此操作将消去小数点前的零
        c += e; //两串相加
    }
    return c;
}
...
string Operation::Module(
    string a, string b) //很简单，利用减法循环相减，不能减了剩下的就是余数
{
    if (!cmp(a, b)) {
        while (!cmp(a, b))
            a = Less(a, b);
...
int main(void)
{
    ...
    char a;
    int n = 0;
    cout << "Choose your method of calculation" << endl;
    cout << "Please print +, -,* ,/ or %" << endl;
    while (true) {
        cin >> str1;
        cin >> a;
        cin >> str2;
